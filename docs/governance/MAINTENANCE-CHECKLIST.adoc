...
- [ ] ABI/FFI policy scan (if applicable: Idris2 ABI).
...
find . -type f \( -name '*.idr' -o -name '*.idris2' \)
```
...
- [x] FFI/ABI purity policy enforced (`*.idr`/Idris2 for ABI).
...
- [x] ABI/FFI file inventory run (`*.idr`).
...
- [x] Non-eclexia `proven` library checked: already Idris2-first with a C ABI bridge; no additional integration changes required in this run.
