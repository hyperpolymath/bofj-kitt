...
// Replace bofj-kitt, bofj-kitt, {{DEPS}} with actuals
= bofj-kitt — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining bofj-kitt for
...
| `$PREFIX/share/bofj-kitt/`
| Data files, assets

| `$PREFIX/share/doc/bofj-kitt/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/bofj-kitt/config.toml`

Fallback: `$HOME/.config/bofj-kitt/config.toml`
...
Or via OPSM: `opsm update bofj-kitt`
...
[source,bash]
----
just install --prefix=/opt/bofj-kitt-instance1 --config=/etc/bofj-kitt/instance1.toml
just install --prefix=/opt/bofj-kitt-instance2 --config=/etc/bofj-kitt/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/bofj-kitt/issues
